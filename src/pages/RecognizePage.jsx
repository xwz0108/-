...
// 根据食物名称估算营养（数据库无匹配时使用）
function estimateNutrition(foodName, totalCalorie) {
  if (!totalCalorie || totalCalorie <= 0) totalCalorie = 200
  const name = foodName.toLowerCase()

  if (/鸡|肉|鱼|虾|牛|羊|猪|排|腿|胸/.test(name)) {
    return calcNutrition(totalCalorie, 0.30, 0.35, 0.35)
  } else if (/沙拉|蔬菜|菜|苹果|香蕉|橙|葡萄|草莓|西瓜|芒果|桃|梨|猕猴桃|菠萝|柚|柠檬|火龙果|木瓜|哈密瓜|西兰花|青菜|菠菜/.test(name)) {
    return calcNutrition(totalCalorie, 0.15, 0.10, 0.75)
  } else if (/汉堡|披萨|薯条|炸鸡|热狗|三明治|面包|蛋糕|甜甜圈|饼干|薯片|意大利面|拉面|饺子|包子|馒头|油条/.test(name)) {
    return calcNutrition(totalCalorie, 0.15, 0.30, 0.55)
  } else if (/牛奶|酸奶|奶酪|豆浆|鸡蛋|煎蛋/.test(name)) {
    return calcNutrition(totalCalorie, 0.25, 0.40, 0.35)
  } else if (/巧克力|冰淇淋|奶茶|啤酒|红酒|咖啡/.test(name)) {
    return calcNutrition(totalCalorie, 0.05, 0.35, 0.60)
  }
  return calcNutrition(totalCalorie, 0.20, 0.30, 0.50)
}

function calcNutrition(totalCalorie, proteinRatio, fatRatio, carbsRatio) {
  return {
    calories: Math.round(totalCalorie),
    protein: Math.round((totalCalorie * proteinRatio) / 4 * 10) / 10,
    fat: Math.round((totalCalorie * fatRatio) / 9 * 10) / 10,
    carbs: Math.round((totalCalorie * carbsRatio) / 4 * 10) / 10,
  }
}
...
function RecognizePage() {
  ...
  const handleRecognize = async () => {
    ...
    try {
      ...
      if (glmResult.error) {
        throw new Error(glmResult.error)
      }
    ...
    <Container maxWidth="md" sx={{ pb: { xs: 10, md: 4 } }}>
      ...
      <Box sx={{ textAlign: 'center', mb: 4, mt: { xs: 2, md: 4 } }}>
        ...
        <Typography variant="h4" component="h1" fontWeight="bold" gutterBottom>
          食物<span style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>智能识别</span>
        </Typography>
        <Typography variant="body1" color="text.secondary">
          上传食物图片，GLM-4.6V AI 自动识别并分析热量营养
        </Typography>
      </Box>
        ...
        <Zoom in timeout={800}>
          <Paper
            ...
            sx={{
              ...
              borderRadius: 4,
              bgcolor: dragActive ? 'rgba(102,126,234,0.05)' : 'white',
              cursor: 'pointer',
          ...
          >
            ...
            <Button
              ...
              sx={{
                ...
                '&:hover': {
                  background: 'linear-gradient(135deg, #5a6fd6 0%, #667eea 100%)',
                ...
                <LinearProgress
                  sx={{
                    ...
                    '& .MuiLinearProgress-bar': {
                      background: 'linear-gradient(90deg, #667eea 0%, #764ba2 50%, #2196F3 100%)',
                      borderRadius: 4,
                    },
                  }}
                />
      ...
      {/* Tips */}
      <Box sx={{ mt: 6, p: 4, bgcolor: 'rgba(102,126,234,0.03)', borderRadius: 4, border: '1px solid rgba(102,126,234,0.1)' }}>
        <Typography variant="h6" gutterBottom color="primary" fontWeight="bold">
          💡 拍照小贴士
        </Typography>
        <ul style={{ margin: 0, paddingLeft: '20px', listStyle: 'none' }}>
          <li style={{ padding: '8px 0', borderBottom: '1px solid rgba(102,126,234,0.1)' }}>
